...
if($dbh) {
	if ($role=='crowd') {
		...
		$sth = $dbh->query("SELECT COUNT(*) AS count FROM whois_online WHERE id='".$id."'");
	...
	$expireTime = time() - 5;
	$dbh->query("DELETE FROM whois_online WHERE (date_part('second', localtime-time) > 5) OR (whois_online.time IS NULL)");	
